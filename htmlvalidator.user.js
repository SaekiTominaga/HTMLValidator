...
(async () => {
	...
	/* ロード中のアイコンデータ */
	const ICON_LOADING_DATA = 'data:image/svg+xml;base64,';
	...
	const CSS = `
		...
		.${CLASSNAME_PLEFIX}icon:enabled {
			cursor: pointer;
		}

		.${CLASSNAME_PLEFIX}icon img {
			display: block;
			height: 40px;
			width: 40px;
		}
		...
		.${CLASSNAME_PLEFIX}section:not(:first-child) {
			margin-top: 1em;
		}

		.${CLASSNAME_PLEFIX}heading {
			margin: 0 0 .1em;
			font-size: inherit;
			font-weight: bold;
		}

		.${CLASSNAME_PLEFIX}list {
		...
		.${CLASSNAME_PLEFIX}validate-type {
			font-weight: bold;
			text-transform: capitalize;
		}

		.${CLASSNAME_PLEFIX}validate-line {
			text-align: end;
		}

		.${CLASSNAME_PLEFIX}validate-message {
		}

		.${CLASSNAME_PLEFIX}message {
			margin: 0;
			padding: .25em .5em;
			border: 1px solid #000;
			background: #fff;
		...
		.${CLASSNAME_PLEFIX}message.--success {
			border-color: #080;
			color: #080;
		}

		.${CLASSNAME_PLEFIX}message.--error {
			border-color: #e00;
			color: #e00;
			font-weight: bold;
		}

		.${CLASSNAME_PLEFIX}message.--error::before {
			margin-inline-end: .25em;
			content: "⚠";
		}

		.${CLASSNAME_PLEFIX}close-button {
			margin-top: 1.5em;
		}
	`;
	...
	try {
		...
		iconButtonElement.className = `${CLASSNAME_PLEFIX}icon`;
		...
		closeButtonElement.className = `${CLASSNAME_PLEFIX}close-button`;
		...
		for (const url of supportGMgetValue ? GM_getValue('CHECKER_URL', CHECKER_URL) : CHECKER_URL) {
			try {
				...
				sectionElement.className = `${CLASSNAME_PLEFIX}section`;
				errorBoxElement.insertBefore(sectionElement, closeButtonElement);

				const headingElement = document.createElement('h2');
				headingElement.className = `${CLASSNAME_PLEFIX}heading`;
				headingElement.textContent = url;
				sectionElement.appendChild(headingElement);

				if (validateMessageList.length === 0) {
					/* メッセージがない時（Success） */
					const messageElement = document.createElement('p');
					messageElement.className = `${CLASSNAME_PLEFIX}message --success`;
					messageElement.textContent = 'No errors or warnings to show.';
					sectionElement.appendChild(messageElement);

					switch (iconElement.src) {
						case ICON_LOADING_DATA:
					...
					tableElement.className = `${CLASSNAME_PLEFIX}list`;
					sectionElement.appendChild(tableElement);

					const tbodyElement = document.createElement('tbody');
					tableElement.appendChild(tbodyElement);

					for (const validateMessage of validateMessageList) {
						const type = validateMessage.subType !== undefined ? validateMessage.subType : validateMessage.type;
						types.push(type);
						...
						typeElement.className = `${CLASSNAME_PLEFIX}validate-type`;
						typeElement.textContent = type;
						trElement.appendChild(typeElement);

						const lineElement = document.createElement('td');
						lineElement.className = `${CLASSNAME_PLEFIX}validate-line`;
						lineElement.textContent = validateMessage.lastLine;
						trElement.appendChild(lineElement);

						const messageElement = document.createElement('td');
						messageElement.className = `${CLASSNAME_PLEFIX}validate-message`;
						messageElement.textContent = validateMessage.message;
				...
				const messageElement = document.createElement('p');
				messageElement.className = `${CLASSNAME_PLEFIX}message --error`;
